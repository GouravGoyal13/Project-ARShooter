Shader "GLU/Multi Layer Animated Skybox" 
{
    Properties 
    {
        _TextureLayer0 ("Texture Layer 0 (Uses UV Channel 1)", 2D) = "white" {}
        _TextureLayer1 ("Texture Layer 1 (Uses UV Channel 2)", 2D) = "black" {}
    ...
    SubShader 
    {
        ...
        Pass
        {
            ...
            VertOut v( float4 position : POSITION, float2 uv : TEXCOORD0, float2 uv2 : TEXCOORD1, float4 color : COLOR  )
            {
                ...
                float2 uvLayer1 = (uv2.xy * _TextureLayer1_ST.xy + _TextureLayer1_ST.zw) + ( _Time.y * _LayerMovement.xy );
