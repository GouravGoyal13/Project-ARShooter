Shader "GLU/Texture/DIFFUSE/Unlit With Rotation And Translation"
{
	Properties
	{
	...
	SubShader 
	{
		...
		Pass
		{
			...
			{
				float theta = radians(  fmod(rotationInDegrees, 360.0 ) );
			...
			VertOut v( float4 vertex : POSITION0, float2 texcoord : TEXCOORD0 )
			{
				VertOut OUT;
			...
			PixelOut p ( VertOut input )
			{
				...
				float3 tintedColor = textureColor.rgb * _Color.rgb;
				float4 finalColor = float4( tintedColor, textureColor.a * _Color.a );
