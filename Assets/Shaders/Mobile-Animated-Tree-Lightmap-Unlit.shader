Shader "GLU/Transparent/Unlit Animated Tree (Supports Lightmap)" 
{
    Properties
    {
        ...
        _MaxSpeed ("Max Speed (X/Y)", Vector) = (2.0, 2.0, 0.0, 0.0)
        _TextureSkewScalar ("Texture Skew Scalar (X/Y)", Vector ) = (30.0, 30.0, 0.0, 0.0)
    }
        ...
        Pass 
        {
            ...
            CGPROGRAM
                ...
                v2f vert (appdata_full v)
                {
                    ...
                    float widthScalar = max( _TextureSkewScalar.x, 0.001 );
                    float heightScalar = max( _TextureSkewScalar.y, 0.001 );
                    // Add the object space vertex z into the mix to give some randomization.
                    float xOffset = (sin( v.vertex.z + (_Time.y * _MaxSpeed.x)) * v.color.r) / widthScalar;
                    float yOffset = (cos( v.vertex.z + (_Time.y * _MaxSpeed.y)) * v.color.r) / heightScalar;
